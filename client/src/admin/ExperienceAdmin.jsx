...
export default function ExperienceAdmin() {
  ...
  const openEdit   = e => {
    ...
    setForm({
      ...
      // skills stored as array in DB → join for input
                ...
                <div className="min-w-0">
                  ...
                  {e.skills?.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {e.skills.slice(0, 4).map(s => <span key={s} className="tag text-xs">{s}</span>)}
                    </div>
                  )}
                </div>
              </div>
        ...
        {modal && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="fixed inset-0 z-40 bg-black/70 backdrop-blur-sm" onClick={() => setModal(false)} />
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid sm:grid-cols-2 gap-4">
                    <Field label="Role *">
                      <input value={form.role} onChange={e => setForm(p => ({ ...p, role: e.target.value }))}
                        placeholder="e.g. Full Stack Developer" className="input-field text-sm" />
                    </Field>
                    <Field label="Company *">
                      <input value={form.company} onChange={e => setForm(p => ({ ...p, company: e.target.value }))}
                        placeholder="e.g. Acme Corp" className="input-field text-sm" />
                    </Field>
                    <Field label="Period">
                      <input value={form.period} onChange={e => setForm(p => ({ ...p, period: e.target.value }))}
                        placeholder="e.g. Jan 2023 – Present" className="input-field text-sm" />
                    </Field>
                  ...
                  <Field label="Description">
                    <textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))}
                      ...
                      placeholder={"Each sentence becomes a bullet point on the public page.\nTip: end each point with a period."} />
                  </Field>
