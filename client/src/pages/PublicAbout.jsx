    ...
    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
        ...
        <div className="flex flex-wrap gap-2">
            {items?.map((item, index) => (
                <span key={index} className="bg-indigo-100 dark:bg-indigo-900 text-indigo-700 dark:text-indigo-300 px-3 py-1 rounded-full text-sm">
                    {item}
                </span>
            ))}
        </div>
    ...
    <div className="p-6 md:p-12 max-w-6xl mx-auto text-gray-900 dark:text-white">
      ...
      <div className="flex flex-col md:flex-row items-center gap-8 mb-12">
        ...
        <div className="text-center md:text-left">
          ...
          <p className="text-xl text-indigo-600 dark:text-indigo-400 mt-1">{data.title}</p>
          {data.tagline && (
       ...
       {data.experience && (
        <div className="mb-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <StatCard icon={<Briefcase className="w-8 h-8 text-indigo-500"/>} value={data.experience.yearsOfExperience || "0"} label="Years Experience" />
              <StatCard icon={<Server className="w-8 h-8 text-indigo-500"/>} value={data.experience.projectsCompleted || "0"} label="Projects Completed" />
              <StatCard icon={<Building2 className="w-8 h-8 text-indigo-500"/>} value={data.experience.companiesWorked || "0"} label="Companies Worked" />
              <StatCard icon={<Users className="w-8 h-8 text-indigo-500"/>} value={data.experience.clientsSatisfied || "0"} label="Clients Satisfied" />
          </div>
        </div>
      )}
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          {data.bio && (
            <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow">
              ...
              <p className="text-gray-700 dark:text-gray-300 leading-relaxed">{data.bio}</p>
            </div>
          )}
                        ...
                        {data.achievements.map((ach, i) => (
                            <li key={i} className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-600 dark:text-gray-400">{ach.description} ({ach.project} - {ach.year})</p>
                                </div>
                            </li>
                        ))}
                    </ul>
        ...
        <div className="space-y-8">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow">
                ...
                <ul className="space-y-3 text-gray-700 dark:text-gray-300">
                    <li className="flex items-center gap-2"><Mail className="w-5 h-5" /> {data.email}</li>
                    <li className="flex items-center gap-2"><Phone className="w-5 h-5" /> {data.phone}</li>
                    <li className="flex items-center gap-2"><MapPin className="w-5 h-5" /> {data.location}</li>
                </ul>
                <div className="flex flex-wrap gap-4 mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    {data.github && <a href={data.github} target="_blank" rel="noopener noreferrer" className="text-gray-800 dark:text-white hover:underline"><Github /></a>}
                    {data.twitter && <a href={data.twitter} target="_blank" rel="noopener noreferrer" className="text-sky-500 hover:underline"><Twitter /></a>}
                    {data.facebook && <a href={data.facebook} target="_blank" rel="noopener noreferrer" className="text-blue-700 hover:underline"><Facebook /></a>}
                    {data.instagram && <a href={data.instagram} target="_blank" rel="noopener noreferrer" className="text-pink-500 hover:underline"><Instagram /></a>}
                </div>
            </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {data.industries.map((industry, i) => (
                            <span key={i} className="bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 px-3 py-1 rounded-full text-sm">{industry}</span>
                        ))}
                    </div>
                </div>
            )}
      ...
      {data.expertise && (
        <div className="mt-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <ExpertiseCard title="methodologies" items={data.expertise.methodologies} />
          </div>
        </div>
      )}
